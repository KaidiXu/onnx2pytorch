...
class Reshape(Operator):
    ...
    def forward(self, input: torch.Tensor, shape=None):
        ...
        if shape[0] == 1 and (len(shape) == 4 or len(shape) == 2) and self.quirks.get("fix_batch_size") is True:
            ...
            if torch.prod(torch.tensor(input.shape)) != torch.prod(shape) and len(input.size()) == len(shape) + 1 and torch.prod(torch.tensor(input.shape)) == input.shape[0] * torch.prod(shape) and self.quirks.get("merge_batch_size_with_channel") is True:
